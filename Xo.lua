...
function Xo:init(args)
  args.title = "Xo"
  args.mnemonic = "XO"
  Unit.init(self,args)
end

function Xo:onLoadGraph(channelCount)
...
function Xo:loadMonoGraph()
    ...
    for i, name in ipairs(opNames) do
        ...
        connect(localDSP["phaseMixer" .. name .. "1"],"Out",localDSP["op" .. name],"Phase")
    end 
...
function Xo:onLoadViews(objects,branches)
    local controls = {}
...
function Xo:changeView(view)
    currentView = view
    self:switchView(view)
end
  
  function Xo:onShowMenu(objects,branches)
    local controls = {}
